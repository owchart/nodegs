 ...
 * @author GAIA
 */
public class LoginService extends HttpPostService {
    public LoginService()
    {
    ...
    public int Send(int functionID, int requestID, List<LoginInfo> loginInfos)
    {
        try
        {
            ...
            int ret = Send(new CMessage(GetGroupID(), GetServiceID(), FUNCTIONID_LOGIN_GETLOGININFOS, GetSessionID(), requestID, 0, 0, GetCompressType(), bytes.length, bytes));
            bw.Close();
            return ret;
        }
        catch(Exception ex)
