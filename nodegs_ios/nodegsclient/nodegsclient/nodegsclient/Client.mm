...
namespace OwLibSock
{
	...
	ConnectStatus Client::ConnectStandard()
	{
    ...
    ConnectStatus Client::ConnectByHttp()
    {
		char buf[512];
		if (m_proxyUserName != "")
		{
    ...
    ConnectStatus Client::ConnectBySock4()
    {
		char buf[512];
    ...
    ConnectStatus Client::ConnectBySock5()
    {
		char buf[512];
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	Client::Client(long proxyType, string ip, u_short port, string proxyIp, u_short proxyPort, string proxyUserName, string proxyUserPwd, string proxyDomain, int timeout)
	{
	...
	ConnectStatus Client::Connect()
	{
		...
		else
		{
			...
			unsigned long ul = 0;
			int ret = fcntl(m_hSocket, F_SETFL, O_NONBLOCK);
			switch(m_proxyType)
			{
                case 1:
	...
	ConnectStatus Client::ConnectProxyServer()
	{
        return CONNECT_PROXY_FAIL;
	}
    
    string Client::GetHostIP(const char* ip)
    {
        char szIP[1024] = {0};
        char *ptr, **pptr;
        struct hostent *hptr;
	...
	int Client::Send(int socketID, const char *str, int len)
	{
		return send(socketID, str, len, 0);
	}
    
	void Client::WriteLog(int socketID, int localSID, int state, const char *log)
	{
		if(m_writeLogEvent)
		{
