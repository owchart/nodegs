...
namespace OwLib
{
	...
	string CurlHttp::DownloadFile(string fileUrl, string filePath, FileTransferListener *progress /*= 0*/, int timeout /*= 600 */)
	{
		m_Lock.Lock();
	...
	string CurlHttp::UploadBitFile(string uploadUrl, string uploadFileName, FileTransferListener *progress /*= 0*/)
	{
		...
		curl_easy_setopt(easy_handle, CURLOPT_PROGRESSFUNCTION, progress ? progress->m_fun : 0); 
		curl_easy_setopt(easy_handle, CURLOPT_PROGRESSDATA, progress ? progress->m_ptr : 0); 
