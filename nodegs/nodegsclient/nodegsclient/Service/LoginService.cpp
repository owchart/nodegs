...
namespace OwLibCT
{
	...
	int LoginService::Login(const String& userName, const String& passWord, int requestID)
	{
	    LoginInfo loginInfo;
	...
	int LoginService::Send(int functionID, int requestID, vector<LoginInfo> *loginInfos)
	{
		...
		for(int i = 0; i < loginInfosSize; i++)
		{
			...
			strSize += sizeof(int) * 4 + CStrA::GetWStringCapacity(loginInfo.m_userName) 
				+ CStrA::GetWStringCapacity(loginInfo.m_passWord)
				+ CStrA::GetWStringCapacity(loginInfo.m_nickName) + 20;
		}
		Binary binary;
