...
namespace OwLibCT
{
	...
	int BaseService::ConnectToServer(int type, int proxyType, const char *ip, int port, const char *proxyIp, int proxyPort, const char *proxyUserName, const char *proxyUserPwd, const char *proxyDomain, int timeout)
	{
	...
	void BaseService::AddService(BaseService *service)
	{
		...
		{
			string path = GetProgramDir() + "\\owsock_client.dll";
			...
			p_funcRegisterCallBack = (funcRegisterCallBack)GetProcAddress(hdll, "RegisterClientCallBack");
			p_funcRegisterLog = (funcRegisterLog)GetProcAddress(hdll, "RegisterClientLog");
